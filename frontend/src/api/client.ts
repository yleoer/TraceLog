...
export const api = {
  ...
  listTimeWorkItems: () => nativeCall(() => DesktopApp.ListTimeWorkItems()) as Promise<TimeWorkItem[]>,
  getTimeWeek: (week = '') => nativeCall(() => DesktopApp.GetTimeWeek(week)) as Promise<TimeWeekView>,
  refreshTimeWeek: (week = '') => nativeCall(() => DesktopApp.RefreshTimeWeek(week)) as Promise<TimeWeekView>,
  logTempoTime: (request: LogTimeRequest) =>
    nativeCall(() => DesktopApp.LogTempoTime(service.LogTimeRequest.createFrom(request))) as Promise<LogTimeResult>,
